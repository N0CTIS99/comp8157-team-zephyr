...
const DataForm: React.FC<DataFormProps> = ({ onFormSubmit }) => {
    ...
    onSubmit: async values => {
      ...
      try {
        ...
        const dataDoc: DataDoc = {
          ...
          gasUsage: `${values.gasUsage} gal`,
          fuelUsage: `${values.fuelUsage} gal`,
          avgMilesDriven: `${values.avgMilesDriven} gal`,
          carbonFootPrint,
  ...
  const calculate = () => {
    const result = `${(
      Number(formik.values.electricUsage) * 0.000417 +
      Number(formik.values.gasUsage) * 0.0057458 +
      Number(formik.values.fuelUsage) * 0.009874 +
      Number(formik.values.avgMilesDriven) * 0.008786
    ).toFixed(3)} metric tons`
          ...
          <form onSubmit={formik.handleSubmit}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <TextField
                  variant="standard"
                  label="Petrol Burned For All Cars Owned"
                  fullWidth
                  type="number"
